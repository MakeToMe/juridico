/**
 * Script para fazer login no TJAL e navegar para a página de consulta
 * Corrigido para tratar corretamente o preenchimento do número do processo
 */
...
/**
 * Função para fazer login no TJAL e navegar para a página de consulta
 */
async function loginEConsultaTJAL() {
  ...
  try {
    ...
    // Dividir o número do processo corretamente
    // Formato: 0727108-89.2024.8.02.0001
    // Precisamos preencher apenas a primeira parte (0727108-89.2024) e a última (0001)
    // O bloco do meio (8.02) já vem preenchido
    const partes = NUMERO_PROCESSO.split('.');
    
    // Extrair a primeira parte (antes do primeiro ponto)
    const numeroPrimeiraParte = partes[0]; // 0727108-89.2024
    
    // Extrair a última parte (depois do último ponto)
    const numeroTerceiraParte = partes[2]; // 0001
    
    console.log(`Preenchendo número do processo:`);
    console.log(`- Primeira parte: ${numeroPrimeiraParte}`);
    console.log(`- Terceira parte: ${numeroTerceiraParte}`);
    
    // Limpar e preencher o primeiro campo
    await page.fill('#numeroDigitoAnoUnificado', '');
    await page.fill('#numeroDigitoAnoUnificado', numeroPrimeiraParte);
    
    // Limpar e preencher o terceiro campo
    await page.fill('#foroNumeroUnificado', '');
    await page.fill('#foroNumeroUnificado', numeroTerceiraParte);
    
    // Verificar o valor do campo do meio (deve ser 8.02)
    const valorCampoMeio = await page.$eval('#local', el => el.value);
    console.log(`Valor do campo do meio: ${valorCampoMeio}`);

 ...
 * e enviar para a API que salvará no Supabase
 */
...
// URL da API para salvar os dados
 ...
 */
async function enviarDadosParaAPI(dadosProcesso, movimentacoes) {
  try {
    console.log('\n=== ENVIANDO DADOS PARA A API ===');
    
    // Adicionar o ID da empresa aos dados do processo
    dadosProcesso.empresaUid = EMPRESA_UID;
    ...
    if (!response.ok) {
      throw new Error(`Erro ao enviar dados para a API: ${result.error || response.statusText}`);
    }
    
    console.log(`\nDados enviados com sucesso para a API!`);
    console.log(`Processo UID: ${result.processoUid}`);
...
async function extrairMovimentacoesTJAL() {
  ...
  try {
    ...
    // ETAPA 5: ENVIAR DADOS PARA A API
    console.log('\n=== ETAPA 5: ENVIANDO DADOS PARA A API ===');
    
    // Verificar se a API está rodando
    console.log('Verificando se a API está rodando...');
    console.log(`URL da API: ${API_URL}`);
    console.log('Para testar este script, certifique-se de que o servidor Next.js está rodando com "npm run dev"');
    ...
    await enviarDadosParaAPI(dadosProcesso, movimentacoesProcessadas);
    ...
    // Enviar dados para a API
    const processoUid = await enviarDadosParaAPI(dadosProcesso, movimentacoesProcessadas);

 ...
 */
async function verificarElementos(page) {
 ...
 */
async function preencherFormulario(page, numeroProcesso) {
  ...
  try {
    await page.evaluate((parte1, parte2) => {
      const campoNumero = document.querySelector('input[name="numeroDigitoAnoUnificado"]');
      const campoForo = document.querySelector('input[name="foroNumeroUnificado"]');
      ...
      const radioUnificado = document.querySelector('input[type="radio"][name="tipoCriterioUnificado"][value="U"]');
 ...
 */
async function verificarBotaoConsulta(page) {
 ...
 */
async function tentarClicarBotao(page) {

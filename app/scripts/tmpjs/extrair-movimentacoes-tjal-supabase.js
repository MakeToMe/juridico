/**
 * Script final para extrair movimentações do TJAL e salvar no Supabase
 * Extrai todas as movimentações e salva no banco de dados
 */
...
// UID da empresa para associar os processos
...
async function salvarProcessoSupabase(dadosProcesso, movimentacoes) {
  console.log('Salvando processo no Supabase...');
  
  try {
    // Definir o schema para o cliente Supabase
    await supabase.rpc('set_schema', { schema: 'alnpp' });
    ...
    if (processoExistente) {
      console.log(`Processo ${dadosProcesso.numeroProcesso} já existe no banco de dados. Atualizando...`);
      ...
      const { error: erroAtualizacao } = await supabase
        ...
        .update({
          ...
          valor_causa: dadosProcesso.valorAcao,
    ...
    if (dadosProcesso.partes && dadosProcesso.partes.length > 0) {
      console.log('Salvando partes do processo...');
      
      // Remover partes existentes para este processo
      const { error: erroRemocaoPartes } = await supabase
        .from('partes_processo')
        .delete()
        .eq('processo_uid', processoUid);
      
      if (erroRemocaoPartes) {
        console.error('Erro ao remover partes existentes:', erroRemocaoPartes);
      }
      
      // Inserir novas partes
      for (const parte of dadosProcesso.partes) {
        const { error: erroInsercaoParte } = await supabase
          .from('partes_processo')
          .insert({
        ...
        if (erroInsercaoParte) {
          console.error('Erro ao inserir parte:', erroInsercaoParte);
        }
      }
    }
    ...
    if (movimentacoes && movimentacoes.length > 0) {
      console.log('Salvando movimentações do processo...');
      
      // Obter movimentações existentes para verificar o que já existe
      ...
      // Criar um conjunto de chaves para verificar movimentações existentes
      ...
      for (const mov of movimentacoes) {
        // Pular movimentações sem descrição
        if (!mov.descricao) continue;
        ...
        const { error: erroInsercaoMovimentacao } = await supabase
          .from('movimentacoes')
          .insert({
            processo_uid: processoUid,
            data: dataFormatada,
        ...
        if (erroInsercaoMovimentacao) {
          console.error('Erro ao inserir movimentação:', erroInsercaoMovimentacao);
        } else {
          novasMovimentacoes++;
        }
      }
      
      console.log(`${novasMovimentacoes} novas movimentações inseridas.`);
    }
    
    console.log('Processo salvo com sucesso no Supabase!');
    return processoUid;
...
async function extrairMovimentacoesTJAL() {
  ...
  try {
    ...
    const processoUid = await salvarProcessoSupabase(dadosProcesso, movimentacoesProcessadas);
    
    if (processoUid) {
      console.log(`Processo salvo com sucesso no Supabase! UID: ${processoUid}`);
    } else {
      console.log('Não foi possível salvar o processo no Supabase.');
    }
...
extrairMovimentacoesTJAL()
  .then(resultado => {
    ...
    if (resultado) {
      console.log(`Processo ${resultado.dadosProcesso.numeroProcesso} extraído e salvo com sucesso.`);
